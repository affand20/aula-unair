...
import id.trydev.aulaunair.ui.materi.adapter.MateriAdapter
...
class MateriActivity : AppCompatActivity(), EasyPermissions.PermissionCallbacks {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        // Kirim file pdf ke firebase storage
        if (requestCode == rcChoose && resultCode == Activity.RESULT_OK && data!=null && data.data!=null) {
            if (data.data != null) {
                ...
                val ref = mStorage.getReference("${matkul}/materi/$fileName.pdf")
                        ...
                        if (it.isSuccessful) {
                            ...
                            mFirestore.collection("matkul")
                                ...
                                .addOnSuccessListener { snapshot ->
                                    snapshot.forEach {
                                        it.reference.collection("materi")
                                            .add(
                                                Materi(
                                                    fileName,
...
//                            mFirestore.collection("matkul")
//                                .document(matkulId.get(0).matkulId.toString())
//                                .collection("materi")
//                                .add(hashMapOf(
//                                    "nama" to fileName,
//                                    "url" to downloadUrl
